...
"""
        ...
        if mode == "copy":
            ...
            return {"current_file_content": new_code, "pylint_report": updated_pylint, "results":  [entry]}
    ...
    def judge_node(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Advanced Judge Agent: Generates intelligent unit tests using LLM and runs them.
        ...
        # Choisir le dossier de travail en fonction du mode

        base_dir = state.get("work_dir", os.path.dirname(current_file))

        test_file = os.path.join(base_dir, f"test_{module_name}.py")
        ...
        # Extraire toutes les fonctions pour guider le LLM

        func_pattern = re.compile(r"^def\s+(\w+)\s*\((.*?)\):", re.MULTILINE)

        functions = func_pattern.findall(code_content)



        # Construire le prompt pour générer des tests intelligents

        funcs_desc = "\n".join([f"- {name}({args})" for name, args in functions])

        prompt = f"""

You are a Python expert. You are given the following Python module:
...
* Generate a separate Python test file using pytest.

* For each function, create at least one unit test with meaningful inputs and expected outputs.

* Tests should validate the logic, not just syntax.

* Use 'import pytest' and 'from {module_name} import *'.

* The code under test is in a file named '{module_name}.py'.

* The output should be only the test file content, no explanations.

"""



        # Appel du LLM si activé

        test_content = ""

        if not state.get("disable_llm", False):

            _res = self._safe_llm_invoke(prompt, "Judge", ActionType.GENERATION)

            if not _res.get("failed", True):

                test_content = _res.get("content", "")

                # Retirer d'éventuels blocs Markdown
                ...
                test_content = "# LLM failed, fallback placeholder tests\nimport pytest\n"

        else:

            # Fallback si LLM désactivé

            test_content = "# LLM disabled, fallback placeholder tests\nimport pytest\n"
        ...
        results = self.tools.run_pytest(test_file)
        ...
        # En mode 'copy', on considère les tests réussis pour la boucle
        ...
        log_experiment(
            ...
            model_used=self.model_name if not state.get("disable_llm", False) else "system-tool",
